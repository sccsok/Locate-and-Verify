...
def to_bw(mask, thresh_binary=15, thresh_otsu=255):
...
def get_bbox(mask, thresh_binary=15, thresh_otsu=255):
...
def extract_face_videos(output_path):
    f_vid_mask = opt.mask
    r_vid_altered = opt.dataset

    r_img_altered = os.path.join(output_path, 'youtube')
    ...
    for vid in os.listdir(f_vid_mask):
        ...
        if os.path.exists(os.path.join(f_vid_mask, vid)):
            for f in os.listdir(os.path.join(f_vid_mask, vid)):
                ...
                image_altered = cv2.imread(os.path.join(r_vid_altered, vid.split('_')[0] + '.mp4', f))
                altered_cropped = extract_face(image_altered, bbox, opt.scale)

                if altered_cropped is not None:
                    altered_cropped = np.concatenate((altered_cropped, blank_img, blank_img), axis=1)

                    cv2.imwrite(os.path.join(r_img_altered, vid, f), altered_cropped)
